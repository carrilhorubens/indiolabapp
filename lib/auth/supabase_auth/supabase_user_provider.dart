...
class IndioProdutosOpticosSupabaseUser extends BaseAuthUser {
  IndioProdutosOpticosSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> indioProdutosOpticosSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = IndioProdutosOpticosSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
